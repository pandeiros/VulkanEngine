...
bool FileManager::LoadOBJWithNormals(const std::string filename, VertexData& vertices, std::vector<uint32_t>& indices, std::vector<glm::vec3>& normals)
{
    vertices.clear();
    indices.clear();
    normals.clear();
...
#endif
    {
        for (const auto &shape : shapes)
        {
            for (const auto &indice : shape.mesh.indices)
            {
                ...
                {
                    normals.push_back({
                        attrib.normals[3 * indice.vertex_index + 0],
                        attrib.normals[3 * indice.vertex_index + 1],
                        attrib.normals[3 * indice.vertex_index + 2] });
                }
